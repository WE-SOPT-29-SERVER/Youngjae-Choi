...
index.js, api router 등등의 폴더링/ 파일생성을 우다다 마쳐준뒤

1. /function 디렉토리에서 `npm run serve` 명령어를 이용하여 로컬에서 서버를 실행시킨다
2. 같은 디렉토리에서 `npm run deploy` 명령어를 사용한다면 서버에 배포를 하게 된다아
3. 
...
### 3)-2 배포후 API 요청을 하는데 `Your client does not have permission to get URL <code>/api/user/signup</code> from this server.` 오류가 난드아 ㅠㅠ
